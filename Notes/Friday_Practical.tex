\documentclass[apj, onecolumn, lengthcheck,superscriptaddress,letterpaper,nofootinbib,nopreprintnumbers]{revtex4}
...
\item In your chosen coordinate system, write down the polarization tensors $e^+_{ab}(\hat \Omega) = \hat m_a\hat m_b - \hat n_a \hat n_b$, $e^\times_{ab} (\hat \Omega)= \hat m_a \hat n_b + \hat n_a \hat m_b$. For further reading, see \cite{AllenRomano:1999}.
\item You now have all the ingredients to write down the antenna beam pattern, 

\begin{equation}
F^A(\hat \Omega) = \frac{1}{2} \frac{\hat p^i \hat p^j}{1+\hat\Omega\cdot\hat p} e^A_{ij}(\hat\Omega) \, ,  
\end{equation}
where $A=+,\times$ is the GW polarization. Specifically, give expressions for $F^+_a$, $F^\times_a$, $F^+_b$, $F^\times_b$.
